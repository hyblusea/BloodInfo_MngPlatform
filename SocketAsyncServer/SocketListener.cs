...
namespace SocketAsyncServer
{
    ...
    {
        public delegate void MsgReceivedHandler(string msgContent, string remoteIP, int remotePort);
        public event MsgReceivedHandler MsgReceived;

        public delegate void ClientReadHandler(string msgContent, string remoteIP, int remotePort);
        public event ClientReadHandler ClientConnected;
                ...
                try
                {
                    ...
                    if (readEventArgs != null)
                    {
                        ...
                        if (ClientConnected != null)
                            ClientConnected("新客户端接入", ((IPEndPoint)s.RemoteEndPoint).Address.ToString(), ((IPEndPoint)s.RemoteEndPoint).Port);
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            ...
            if (e.BytesTransferred > 0)
            {
                if (e.SocketError == SocketError.Success)
                {
                    ...
                    {
                        // Set return buffer.
                        string sContent = token.ProcessData(e);
                        if (MsgReceived != null)
                        {
                            MsgReceived(sContent, ((IPEndPoint)s.RemoteEndPoint).Address.ToString(), ((IPEndPoint)s.RemoteEndPoint).Port);
                        }
                    }
